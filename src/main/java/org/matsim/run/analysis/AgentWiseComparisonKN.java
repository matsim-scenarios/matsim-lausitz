...
//												  } )
//												  .build();
		// yyyy obiges geht nicht weil die matsim version zu alt ist.  --> diese Klasse nach matsim-libs (vsp) verschieben.  kai, dec'25
...
//		for( Column<?> column : baseTableTrips.columns() ){
...
//		System.out.println( baseTableTrips );

//		{
//			final StringColumn columnToAdd = baseTableTrips.stringColumn( PERSON_ID ).concatenate( "-" ).concatenate( baseTableTrips.intColumn( TRIP_IDX ).asStringColumn() );
//			columnToAdd.setName( "abc" );
//			System.out.println( columnToAdd );
//			baseTableTrips.addColumns( columnToAdd );
//		}
//		{
//			final Table sortedTable = baseTableTrips.sortOn( "-" + SCORE, PERSON_ID, TRIP_IDX );
...
//			double sumAscs = 0.;
			...
			for( TripStructureUtils.Trip trip : trips ){
				...
				processMuM( isBaseTable, person, table );
...
//							}
	...
	private static void processMuM( boolean isBaseTable, Person person, Table table ){
		if ( isBaseTable ){
...
//		Table tripsTablePolicy = generateTripsTableFromPopulation( policyPopulation, config, false );
		Table personsTablePolicy = generatePersonTableFromPopulation( policyPopulation, config, false );

//		System.out.println( tripsTablePolicy );
//
//		printSpecificPerson( tripsTablePolicy );
//
//		tripWiseDifferenceTable( tripsTableBase, tripsTablePolicy );
